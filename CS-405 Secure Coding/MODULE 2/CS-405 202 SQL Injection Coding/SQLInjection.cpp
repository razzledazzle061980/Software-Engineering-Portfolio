...
/* Function removes all escaped characters from the query
 * and then insert quotes at the beginning and end of the
 * parameters.
*/
string escape_sql(string input, string quote) {

    // Loop through input starting at position 0
    for (size_t pos = 0; ; pos += quote.length())
    {
        // Find an escaped character and get its position
        pos = input.find(quote, pos);

        // If the position is at the end or input is empty exit loop
        if (pos == string::npos || input.empty())
            break;
        else 
			input.erase(pos, quote.length());

    }
    // Find the first equals which is where the first quote will be placed
    const size_t first_quote = input.find_first_of('=') + 1;

    // Find the last position to place the last quote
    const size_t last_quote = input.length();

    // Insert the first and last quote
    input.insert(first_quote, quote);
    input.insert(last_quote, quote);
    return input;
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
	...
	// Find an or in the sql query
	if (sql.find("or") != string::npos)
	{

		// Call function to remove escaped characters
        const string escaped = escape_sql(sql, "\'");

        // Create output that shows the original query
        // then a notice of possible injection and finally
        // the escaped string
		cout << endl;
        cout << "Query:  " << sql.c_str() << endl;
	    cout << "Detected an OR in the query and possible SQL Injection" << endl;
	    cout << "Query after escaping:  " << escaped << endl;
	    return false;
	}
