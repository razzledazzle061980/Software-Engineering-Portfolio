package com.gameroom.gameauth.controller;
...
import com.gameroom.gameauth.auth.GameUser;
import com.gameroom.gameauth.dao.GameUserDB;
import com.gameroom.gameauth.representations.GameUserInfo;
...
public class GameUserRESTController {
    ...
    // Sets the USER role is allowed and must be authenticated to get user
    @RolesAllowed({"USER"})
    @GET
    @Path("/{id}")
    ...
    // Sets the ADMIN role is allowed and must be authenticated to create user
    @RolesAllowed({"ADMIN"})
    @POST
